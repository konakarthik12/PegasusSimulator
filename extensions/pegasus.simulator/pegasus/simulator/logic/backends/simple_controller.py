...
class SimpleController(Backend):
    ...
    def __init__(self, 
        ...
        self.reference_change_flag = False
    ...
    def position_controller_tuning(self):
        """
        Set Kp, Ki, and Kd for position controller
        """
        Kp=[10, 10, 10]
        Kd=[8.5, 8.5, 8.5]
        Ki=[4, 4, 4]
    ...
    def velocity_controller_tuning(self):
        """
        Set Kp, Ki, and Kd for velocity controller
        """
        Kp=[2, 2, 2]
        Kd=[.1, .1, .1]
        Ki=[0, 0, 0]
    ...
    def stop(self):
        ...
        try:
            ...
            print("While stopping sim got error : ",e)
        self.reset_control_reference()
        self.reset_statistics()
    ...
    def update_state(self, state: State):
        ...
        """
        self.p = state.grippers_position #state.position
    ...
    def reset_control_reference(self):
        if self.verbose:
            print("reseting the the current control references: ",self.int, self.dif)
        self.int = np.array([0.0, 0.0, 0.0])
        self.dif = np.array([0.0, 0.0, 0.0])

    def update(self, reference, dt: float, track_position: bool = False):
        ...
        Args:
            ...
            reference (p, v, a) each is np.array with dim (3,) and are reference for drone motion
        """
        ...
        # else:
        s = 0.6
        v_ref = reference["v"] # self.d_pd(self.total_time, s, self.reverse)
        p_ref = reference["p"] # self.pd(self.total_time, s, self.reverse)
        a_ref = reference["a"] # self.dd_pd(self.total_time, s, self.reverse)
        j_ref = 0.0 #self.ddd_pd(self.total_time, s, self.reverse)
        yaw_ref = -1.57 +  reference["yaw"] #math.atan2(p_ref[1],p_ref[0]) ## because body frame is rotated 90, and x is not front facing of the body  #self.yaw_d(self.total_time, s)
        ...
        if self.verbose:
            ...
            print("control integration : ",self.int)
            print("control differential : ", self.dif)
        # Compute the tracking errors
        if track_position:
            # if not self.reference_change_flag:
            #     self.int = np.array([0.0, 0.0, 0.0])
            #     self.dif = np.array([0.0, 0.0, 0.0])

            # self.reference_change_flag = True
            ep = self.p-p_ref #self.p + v_ref*dt # self.p-(self.p-vref*dt)
            self.dif = self.v - v_ref
            ev = self.dif 
            ...
            # if self.reference_change_flag:
            #     self.int = np.array([0.0, 0.0, 0.0])
            #     self.dif = np.array([0.0, 0.0, 0.0])
            # # Going back to velocity tracking
            # self.reference_change_flag = False
            ep = self.v-v_ref #self.p + v_ref*dt # self.p-(self.p-vref*dt)
            ev = (self.v-self.dif)/dt-a_ref
            # ev = self.dif
            self.int = self.int +  (ep * dt)
            ei = self.int
            self.dif = self.v
